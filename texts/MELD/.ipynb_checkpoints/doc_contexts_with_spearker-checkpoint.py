...
def deter_contexts(utters, uid):
    ...
    if index - 2 >= 0 and index + 2 < len(utters):
        pre_contexts = [utters[index-2]['speaker'], utters[index-2]['text'], utters[index-1]['speaker'], utters[index-1]['text']]
        
        below_contexts = [utters[index+1]['speaker'], utters[index+1]['text'], utters[index+2]['speaker'], utters[index+2]['text']]
        
        cur_text = '</s>' + ' ' + cur_speaker + ' ' + cur_text + ' ' + '</s>'
    elif index - 2 >= 0:
        pre_contexts = [utters[index - 2]['speaker'], utters[index - 2]['text'], utters[index - 1]['speaker'], utters[index - 1]['text']]
        cur_text = '</s>' + ' ' + cur_speaker + ' ' + cur_text
        if index+1<len(utters):
            below_contexts = [utters[index + 1]['speaker'], utters[index + 1]['text']]
            cur_text = cur_speaker + ' ' + cur_text + ' ' + '</s>'
    elif index + 2 < len(utters):
        below_contexts = [utters[index + 1]['speaker'], utters[index + 1]['text'], utters[index + 2]['speaker'], utters[index + 2]['text']]
        cur_text = '</s>' + ' ' + cur_speaker + ' ' + cur_text + ' ' + '</s>'
        print('before cur_text:{}'.format(cur_text))
        if index - 1 >= 0:
            pre_contexts = [utters[index - 1]['speaker'], utters[index - 1]['text']]
            # cur_text = cur_speaker + ' ' + cur_text
...
def data_process(path, start_id):
    ...
    for i in range(lens):
        ...
        if diag_id == '-1_-1_-1':
            ...
            break
        utters = loc_doc(diag_id, test_dict)
        if len(utters)!=0:
            for utter in utters:
                uid = utter['uid']
    ...
    df.to_csv('./new-meld-test-label-v4-contexts-with-speaker.csv', index=0)
